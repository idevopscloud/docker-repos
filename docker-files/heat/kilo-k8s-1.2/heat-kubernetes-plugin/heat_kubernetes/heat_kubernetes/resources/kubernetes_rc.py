...
class KubernetesReplicationController(resource.Resource):
    ...
    def __gen_new_rc(self, spec):
        post_fix = '-r' + ''.join(
            map(lambda xx:(hex(ord(xx))[2:]),os.urandom(2)))
        spec['metadata']['name'] += post_fix

        if spec.get('spec', {}).get('selector', {}).get('internalID'):
            spec['spec']['selector']['internalID'] += post_fix
        if spec.get('spec', {}).get('selector', {}).get('name'):
            spec['spec']['selector']['name'] += post_fix
        if spec.get('metadata', {}).get('labels', {}).get('internalID'):
            spec['metadata']['labels']['internalID'] += post_fix
        if spec.get('spec', {}).get('template', {}).get(
                'metadata', {}).get('labels', {}).get('internalID'):
            spec['spec']['template']['metadata']['labels']['internalID'] += post_fix
        if spec.get('spec', {}).get('template', {}).get(
                'metadata', {}).get('labels', {}).get('name'):
            spec['spec']['template']['metadata']['labels']['name'] += post_fix
    ...
    def handle_create(self):
        ...
        # so we need to create a same RC with different name no matter
        # whether the RC is exist or not(deleted in update case) now.

        if (self.stack.status_reason in ['Stack ROLLBACK started']):
            rc_name, content = self.__gen_new_rc(content_json)
        ...
        try:
            ...
            if (type(e.message) is dict and
                ...
                if (self.stack.status_reason in
                    ...
                    rc_name, content = self.__gen_new_rc(content_json)
                    client.CreateReplicationController(content, self.ns)
                else:
                    # resize it
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        if prop_diff:
            old_def = copy.deepcopy(self.properties.data['definition'])
            ...
            if self.new_rc_name == self.old_rc_name:
                LOG.info(_("[%s] %s: Same RC name [%s]."
                           % (self.stack.name, fname, self.old_rc_name)))
                if self.old_rc_replicas != new_rc_desired_replicas:
                    tmp = copy.deepcopy(old_def)
                    tmp['spec']['replicas'] = new_rc_desired_replicas
                    if tmp == prop_diff['definition']:
                        LOG.info(_("Will resize RC [%s]: %d -> %d."
                                   % (self.old_rc_name, self.old_rc_replicas,
                                      new_rc_desired_replicas)))
                        client.ResizeReplicationController(
                            name=self.old_rc_name,
                            replicas=new_rc_desired_replicas, namespace=self.ns)
                    else:
                        msg = _("Same RC name with different defination except "
                                "for replicas! old_def[%s], new_def[%s]."
                                % (old_def, prop_diff['definition']))
                        LOG.error(msg)
                        raise exception.StackValidationFailed(message=msg)
                else:
                    LOG.info(_("RC[%s] replicas[%d] not change, do nothing."
                               % (self.old_rc_name, self.old_rc_replicas)))
                # no need to change resource id
                return prop_diff
    ...
    def check_update_complete(self, updater):
        ...
        if not self._has_incomplete_update_process(client):
            if self.old_rc_name == self.new_rc_name:
                # for change replicas case only
                return True
            else:
                if len(self.update_list) == 0:
                    if not client.GetReplicationController(
                            name=self.old_rc_name, namespace=self.ns):
                        # for diff rc name case: 
                        # oldrc deleted && newrc replicas up to desired value
